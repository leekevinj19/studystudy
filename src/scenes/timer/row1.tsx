...
const Row1 = () => {
  ...
  const [durations, setDurations] = useState(() => {
    const saved = sessionStorage.getItem('pomodoroSettings');
    return saved ? JSON.parse(saved) : { work: 25, break: 5 };
  });
  const [sessions, setSessions] = useState<Session[]>(() => {
    const saved = sessionStorage.getItem('pomodoroHistory');
    return saved ? JSON.parse(saved) : [];
  });
  const [endTime, setEndTime] = useState<number | null>(() => {
    const saved = sessionStorage.getItem('pomodoroEndTime');
  ...
  useEffect(() => {
    if (!isRunning) {
      setTimeLeft(isBreak ? durations.break * 60 : durations.work * 60);
    }
  }, [durations, isRunning, isBreak]);

  // Persist timer state to sessionStorage on changes
  useEffect(() => {
    sessionStorage.setItem('pomodoroTimeLeft', timeLeft.toString());
  }, [timeLeft]);

  // Sync durations state from sessionStorage periodically to reflect external changes
  useEffect(() => {
    const interval = setInterval(() => {
      const saved = sessionStorage.getItem('pomodoroSettings');
      if (saved) {
        const settings = JSON.parse(saved);
        setDurations((prev: { work: number; break: number }) => {
  ...
  useEffect(() => {
    sessionStorage.setItem('pomodoroSettings', JSON.stringify(durations));
  }, [durations]);

  useEffect(() => {
    sessionStorage.setItem('pomodoroHistory', JSON.stringify(sessions));
  }, [sessions]);

  useEffect(() => {
    if (endTime !== null) {
      sessionStorage.setItem('pomodoroEndTime', endTime.toString());
    } else {
      sessionStorage.removeItem('pomodoroEndTime');
    }
  }, [endTime]);
      ...
      timer = setInterval(() => {
        ...
        if (endTime) {
          ...
          if (diff >= 0) {
            ...
            setIsBreak(!isBreak);
            setTimeLeft(isBreak ? durations.work * 60 : durations.break * 60);
            setIsRunning(true);
            setEndTime(Date.now() + (isBreak ? durations.work * 60 : durations.break * 60) * 1000);
          }
        }
      }, 1000);
    }
  ...
  }, [isRunning, timeLeft, isBreak, durations, sessions, endTime]);
  ...
  const handlePause = () => {
    // Pause timer without resetting timeLeft
    setIsRunning(false);
  };
  ...
  const clearHistory = () => {
    setSessions([]);
    sessionStorage.removeItem('pomodoroHistory');
  };

  const deleteSession = (id: string) => {
    const updatedSessions = sessions.filter(session => session.id !== id);
    setSessions(updatedSessions);
    sessionStorage.setItem('pomodoroHistory', JSON.stringify(updatedSessions));
  };
